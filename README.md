# Clepsydra

Помодоро-таймер в меню-баре macOS: 25 минут работы, 5 минут перерыва и
полноэкранный экран с цитатой между ними. Длины меняются в подменю
«Длительность» — сессия 15, 25 или 45 минут, перерыв 5, 10 или 15.

Приложение живёт в меню-баре: иконка, отсчёт до конца интервала и короткое
меню. В Dock и переключателе задач его нет.

![Меню в меню-баре](docs/images/menubar.png)

Тем, кто забывает начать, в меню есть «Напоминать о сессии»: раз в пятнадцать
минут простоя приходит уведомление, и сессия запускается прямо из него. По
умолчанию напоминание выключено, а Mac, за которым никого нет, оно не окликает.
Баннер macOS убирает сама через несколько секунд; чтобы напоминание дожидалось
щелчка, в «Системных настройках» (System Settings) → «Уведомления»
(Notifications) → Clepsydra выберите стиль «Предупреждения» (Alerts).

Когда сессия кончилась, на всех мониторах разворачивается экран с цитатой
и кнопкой «Отдохнуть».

![Экран в режиме философов](docs/images/philosophers.png)

Второй режим показывает вместо философов пацанские цитаты наклейками.
Фотографий в репозитории нет, права на них не наши: свои PNG кладут
в `Resources/statham/`.

![Экран в режиме Стетхема](docs/images/statham.png)

## Установка

1. **[Скачайте Clepsydra](https://github.com/ilya-pesterev/clepsydra/releases/latest/download/Clepsydra.dmg)** —
   образ `Clepsydra.dmg` начнёт скачиваться сразу, около 11 МБ. Нужен Mac на
   Apple Silicon и macOS 14 или новее. Какая это версия, что изменилось и
   контрольная сумма — на
   [странице релиза](https://github.com/ilya-pesterev/clepsydra/releases/latest).
2. Откройте образ двойным щелчком и перетащите иконку Clepsydra на ярлык
   «Applications» — приложение скопируется в папку «Программы». Дальше
   запускайте его из Launchpad.
3. **Первый запуск macOS остановит**: сертификата Apple Developer ID у проекта
   нет, и подтвердить сборку ей нечем.

   В окне «Файл «Clepsydra» не был открыт» (“Clepsydra” Not Opened) нажмите
   «Готово» (Done). Синяя кнопка рядом — «Переместить в Корзину» (Move to
   Trash), не нажимайте Enter не глядя.

   Дальше  → «Системные настройки…» → «Конфиденциальность и безопасность»
   (Privacy & Security): рядом со строкой про заблокированный файл будет кнопка
   «Все равно открыть» (Open Anyway). Второй запуск переспросит в последний
   раз — «Открыть» (Open), и больше приложение ни о чём не спросит.

Терминал не нужен ни разу. Весь путь подробно, вместе с запуском при входе и
разбором того, чего быть не должно, — в
[руководстве по установке](docs/install.md).

Этот путь проходят один раз. Дальше приложение обновляется само: когда выйдет
новая версия, в меню появится «Обновить до 1.1» — щелчок скачает её, заменит
установленную и запустит уже новую.

## Разработка

[Как всё устроено, собирается и выпускается](docs/development.md) — сборка идёт
на одних Command Line Tools, полный Xcode не нужен.

[CONTEXT.md](CONTEXT.md) — язык проекта, [docs/adr/](docs/adr/) — решения.
